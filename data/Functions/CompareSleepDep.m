function CompareSleepDep(datafile300_week1, datafile1000_week4, datafilesleepdep, convert_ZT)
    ...
    b.CData(3,:) = [0.5, 0.2, 0.2]; % Color for SleepDep
    ...
    % Add asterisk and lines for first comparison (300 Lux Week 1 vs SleepDep)
    if p1 < 0.05
        plot([1, 3], [y_max, y_max], '-k', 'LineWidth', 1.5); % Corrected the indices for SleepDep comparison
    ...
    % Add asterisk and lines for second comparison (1000 Lux Week 4 vs SleepDep)
    if p2 < 0.05
        y_max2 = y_max * 1.15;
        plot([2, 3], [y_max2, y_max2], '-k', 'LineWidth', 1.5); % Corrected the indices for SleepDep comparison
        plot([2 2], [y_max2 * 0.95, y_max2], '-k', 'LineWidth', 1.5); % Left notch
        plot([3 3], [y_max2 * 0.95, y_max2], '-k', 'LineWidth', 1.5); % Right notch
        text(2.5, y_max2 * 1.05, '*', 'FontSize', 20, 'HorizontalAlignment', 'center');
