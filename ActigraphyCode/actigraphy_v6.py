#!/usr/bin/env python3
# includes multiprocessing capabilities and GUI, no command line progress or Manager/Tracker stuff for multiprocessing
...
from multiprocessing import Pool, cpu_count  # Importing multiprocessing
...
class ActigraphyProcessorApp(QWidget):
    ...
    def start_actigraphy(self):
        ...
        if video_file:
            ...
            # Use multiprocessing for processing the folder
            self.worker = Worker(self.actigraphy_processor.process_video_files_multiprocessing,
...
class ActigraphyProcessor:
    ...
    def process_video_files(self, video_folder, oaf, set_roi, name_stamp, output_directory, progress_callback=None):
        ...
        # Helper function to process a single file and update progress
        def process_single_file(file):
            self.process_single_video_file(file, name_stamp, set_roi, output_directory, None, self.roi_pts)

        total_files = len(all_mp4_files)
        files_processed = 0

        with Pool(processes=cpu_count()) as pool:
            for _ in pool.imap_unordered(process_single_file, all_mp4_files):
                files_processed += 1
                if progress_callback:
                    folder_progress = int((files_processed / total_files) * 100)
                    progress_callback.emit(folder_progress)
    ...
    def _get_creation_time_from_name(filename):
        # Updated regex pattern and datetime format
    ...
    def process_single_file(self, file, name_stamp, set_roi, output_directory):
        self.process_single_video_file(file, name_stamp, set_roi, output_directory, None, self.roi_pts)
      
    @staticmethod
    def process_single_file_wrapper(args):
        file, name_stamp, set_roi, output_directory = args
        processor = ActigraphyProcessor()
        processor.process_single_file(file, name_stamp, set_roi, output_directory)

    def process_video_files_multiprocessing(self, video_folder, oaf, set_roi, name_stamp, output_directory, progress_callback=None):
        nested_folders = self.get_nested_paths(video_folder)
        all_mp4_files = [
            os.path.join(folder, mp4_file)
            for folder in nested_folders
        ...
        with Pool(processes=cpu_count()) as pool:
            for _ in pool.imap_unordered(self.process_single_file_wrapper, [(file, name_stamp, set_roi, output_directory) for file in all_mp4_files]):
                files_processed += 1
                if progress_callback:
                    folder_progress = int((files_processed / total_files) * 100)
                    progress_callback.emit(folder_progress)
